...
bool place_in_hash_tables (string s) {
  ...
  while((!placed ) && (counter < 2*tablesize)) {
    if (t[pos][index] == "") {
      ...
      temp = t[pos][index];
      t[pos][index] = temp_s;
      // Store temp into temp_s
      temp_s = temp;


      // Checking/setting the index
      if(index == 0){
        index = 1;
      }else {
        index = 0;
...
size_t f(string s, size_t index) {
  ...
  if (index == 0) {
    ...
    val = s[len - 1]; // Start from the end
    val = val % tablesize;
    if (val < 0) {
      val += tablesize;
    }
    ...
    // Loops through to obtain the hash value and then brings
    // it down to the correct size of 0 to table size - 1
    for (i = len - 2; i >= 0; i--) // Second to last
    {
      temp = s[i];
      po *= prime;
